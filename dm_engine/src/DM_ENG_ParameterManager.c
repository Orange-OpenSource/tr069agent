...
// File d'attente des notifications syst�me
...
int DM_ENG_ParameterManager_init(const char* dataPath, DM_ENG_InitLevel level)
{
   ...
   if (level != DM_ENG_InitLevel_FACTORY_RESET)
   {
      ...
      if (res == 0)
      {
         ...
         {
            // remise � z�ro pour forcer r�initialisation syst�me au d�marrage
   ...
   if (level == DM_ENG_InitLevel_FACTORY_RESET)
   {
      DM_ENG_deleteAllSystemNotificationStruct(&pendingChanges); // au cas o� il resterait des notif en attente
   }
...
static int _actionAfterDownload(const char* fileType, const char* fileName)
{
   ...
   if ((fileType != NULL) && (strcmp(fileType, DATA_MODEL_EXTENSION_FILE_TYPE) == 0))
   {
      ...
      if (res == 0) // on red�marre le module de stats
      {
...
/*
 * on d�place tous les �v�nements en attente vers events
 */
void DM_ENG_ParameterManager_manageEvents(DM_ENG_EventStruct* pendingEvt)
{
   ...
   {
      if (!DM_ENG_ParameterData_isInEvents(DM_ENG_EVENT_PERIODIC)) // EVENT_PERIODIC n'est pas pr�sent, on le rajoute
      {
...
/*
 * Met � jour la liste des �v�nements en rajoutant ou supprimant au besoin le VALUE_CHANGE_EVENT
 * @param valueChanged Indique si VALUE_CHANGE_EVENT doit �tre pr�sent
 */
void DM_ENG_ParameterManager_updateValueChangeEvent(bool valueChanged)
{
   ...
   {
      if (!valueChanged)
      // La notification de ce changement a d�j� �t� faite ds un autre Inform Message (Inform Message p�riodique)
      {
         DM_ENG_ParameterData_deleteEvent(DM_ENG_EVENT_VALUE_CHANGE);
      }
   }
   else if (valueChanged)
      // au moins une valeur chang�e mais events ne contient pas VALUE_CHANGE_EVENT => on le rajoute
   {
...
/*
 * Compl�te la liste des �v�nements avec les "M Download" correspondants aux r�sultats de transfert
 */
...
/*
 * Supprime tous les �v�nements "M Download" de la liste events
 */
...
/*
 * Teste si la liste des �v�nements est vide
 */
...
/*
 * Fournit un EventStruct*[] nouvellement cr��.
 * L'appelant a la charge d'effectuer un free du tableau apr�s utilisation.
 */
...
/*
 * Vide les events (appel� une fois que la transmission � l'ACS a �t� confirm�)
 */
void DM_ENG_ParameterManager_clearEvents()
{
   if (DM_ENG_ParameterData_getEvents() != NULL)
   {
      ...
      // on supprime �galement les flags associ�s � l'�v�nement VALUE CHANGE
      DM_ENG_Parameter* param;
      for (param = DM_ENG_ParameterData_getFirst(); param!=NULL; param = DM_ENG_ParameterData_getNext())
      {
...
static char* _getValue(char* prmName, const char* destName)
{
   ...
   if (prm != NULL)
   {
      if ((prm->storageMode == DM_ENG_StorageMode_DM_ONLY) && DM_ENG_Parameter_isValuable(longName))
      {
         ...
         // cha�ne NULL ou vide consid�r�e comme false
         {
...
/*
 * R�cup�re le param en base apr�s avoir cr�� de nouvelles instances si n�cessaire
 */
static DM_ENG_Parameter* _getParameter(const char* paramName)
{
   ...
   int nbIter;
   char* proto = NULL;
   for (nbIter=0; nbIter<5; nbIter++) // 5 it�rations max pour �viter les boucles infinies (normalement impossibles)
   {
      res = DM_ENG_ParameterData_getParameter(paramName);
      if (res != NULL) return res; // pas de pb, on a trouv� le param�tre en base ; sinon des instances sont � ajouter

      // on recherche le proto � instancier en supprimant it�rativement les n� d'instance de droite � gauche
      // Ex : aa.bb.3.cc.4.dd.2.ee pourra �tre instanci� � partir de aa.bb.3.cc..dd..ee -> instance n�4 de aa.bb.3.cc.
      // puis r�cursivement, � partir de aa.bb.3.cc.4.dd..ee -> instance n� 2 de aa.bb.3.cc.4.dd.
      proto = strdup(paramName);
      unsigned int instanceNb;
      int k = strlen(proto)-1;
      do
      {
         while ((k>0) && (proto[k] != '.')) { k--; } // on cherche le '.' le plus � droite
         if (k<=1) goto fin; // pas trouv�
         int i = k-1;
         while ((i>0) && (proto[i] >= '0') && (proto[i] <= '9')) { i--; } // on remonte tous les chiffres
         if (i<=0) goto fin; // pas trouv�
         if (proto[i] != '.') { k = i-1; continue /*do while*/; } // ce n'est pas une partie chiffr�e
         if (k-i==1) goto fin; // nom incorrect ("..") -> pas trouv�
         proto[k] = '\0';
         if (!DM_ENG_stringToUint(proto+i+1, &instanceNb)) goto fin; // valeur incorrecte -> pas trouv�
         proto[k] = '.';
         if (proto[k+1] == '\0') // queue de cha�ne vide -> ne pas terminer par ".."
         {
            proto[i+1] = '\0';
         }
         else
         ...
         if (DM_ENG_ParameterData_getParameter(proto) != NULL) // eureka ! le proto est d�fini
         {
            proto[i+1] = '\0'; // proto fournit l'objet � instancier
            break /*do while*/;
         }
         k = i-1; // on passe le '.' en i car on n'a jamais 2 parties chiffr�es cons�cutives
      }
      while (true);
      ...
      DM_ENG_FREE(proto);
      // puis on retente r�cursivement. On aura autant d'it�rations que de niveaux d'instanciation n�cessaires
   }
...
// Marque les sous-param de name r�f�renc�s dans names
// Supprime les sous-param de name non r�f�renc�s dans names (non remont�es par Device_getObject() ou Device_getNames())
...
debut:
   for (prmName = DM_ENG_ParameterData_getFirstName(); prmName!=NULL; prmName = DM_ENG_ParameterData_getNextName())
   {
        ...
        // filtre les sous-noeuds de name qui ne soient pas des protos
      {
         ...
         {
            // si c'est un objet de type instance qui n'a pas �t� trouv�, on peut le supprimer
            // (Les param�tres fixes du data model m�me s'ils ne sont pas remont�s ne sont pas supprim�s)
   ...
   if (aSupprimer != NULL)
   {
      ...
      goto debut; // dans le cas o� il y a eu un objet supprim�, on recommence
   }
}

static void _updateSystemValue(DM_ENG_Parameter* param, char* value, bool newValue)
{
   DM_ENG_Parameter_updateValue(param, (value == NULL ? (char*)DM_ENG_EMPTY : value), newValue); // NULL interpt�t� comme EMPTY
}
...
// Retourne 0 si OK, 1 si aucun changement n'est apport� au data model et 900X en cas d'erreur
static int _loadNames(const char* name, const char* data)
{
   // On charge de le data model instanci� depuis le syst�me et on met � jour les donn�es de DM
...
// effectue la mise � jour du param�tre syst�me fourni, noeud ou feuille
    ...
    && (strstr(name, "..") == 0)) // param n'est pas un proto (sinon rien � faire)
   {
      ...
      if (withValues)
      {
         if (isNode)
         {
            ...
            DBG("_loadFinal -> DM_ENG_Device_getObject");
         ...
         {
            DBG("_loadFinal -> DM_ENG_ParameterManager_loadLeafParameterValue");
      ...
      if ((res == 0) || (res == 1)) // cas 1 : le data model est rest� inchang� toutefois on marque le param comme �tant � jour
      {
         param = DM_ENG_ParameterData_getParameter(name); // on r�charge le param car les load ont pu modifier le pointeur
         if (param != NULL)
         {
   ...
   for (prmName = DM_ENG_ParameterManager_getFirstParameterName(); prmName!=NULL; prmName = DM_ENG_ParameterManager_getNextParameterName())
   {
      ...
      if ((prmLen > objLen) && (strncmp(prmName, objName, objLen)==0))
      {
         if (DM_ENG_Parameter_isValuable(prmName))
         {
            ...
            {
               // on r�initialise les param R/O + �ventuellement DiagnosticsState � "None"
               DM_ENG_Parameter_resetValue(param);
            }
         }
         else if (DM_ENG_Parameter_isNodeInstance(prmName))
         {
            // Les objets de type instance doivent �tre supprim�s
            aSupprimer = strdup(prmName);
            break;
         }
      }
...
// le param fourni doit �tre COMPUTED
static int _loadComputedParameter(DM_ENG_Parameter* param, bool checkChange)
{
   ...
   {
      DM_ENG_Parameter_markBeingEvaluated(param); // pour �viter les boucles infinies

      if (*param->definition == '#') // compteur d'�l�ments, pas besoin des valeurs
      {
         char* longName = DM_ENG_Parameter_getLongName(param->definition+1, prmName);
         prmVal = _computeNumberOfIntances(longName);
         free(longName);
      }
      else // expression computed � calculer
      {
   ...
   {
      _updateSystemValue(param, prmVal, checkChange); // si checkChange == false, le flag VALUE CHANGE n'est pas modifi� (s'il �tait true, il le reste)
   }
...
static void _checkComputedChanged(DM_ENG_Parameter* param)
{
   ...
   if (changed)
   {
      ...
      if ((param->value != NULL) || (param->notification == DM_ENG_NotificationMode_PASSIVE) || (param->notification == DM_ENG_NotificationMode_ACTIVE))
        // on calcule la nouvelle valeur et on confirme ou non qu'elle a bien chang�
      {
         _loadComputedParameter(param, true); // positionnera VALUE_CHANGED si la valeur a bien chang�
      }
      else
      {
...
static bool _createParameter(char* newName, char* destName, DM_ENG_ParameterType type, char* args, char* configKey)
{
   ...
   if (prm == NULL)
   {
      prm = DM_ENG_newDefinedParameter(longName, type, args, true); // param�tre interne (= hidden)
...
// parcourt la liste des "sous-param�tres" pour effectuer les mises � jour des param�tres syst�me
...
static int _loadSubParameters(DM_ENG_Parameter* param, bool withValues)
{
   ...
   if ((param==NULL) || (param->name[strlen(param->name)-1] == '.'))
   {
      ...
      {
         DBG("loadingMode = 1 -> _loadFinal");
         res = _loadFinal(param, withValues);
      }
      else // it�rer pour trouver un �l�ment � mettre � jour
      {
         if (param != NULL) { DM_ENG_Parameter_markLoaded(param, withValues); } // on marque le noeud comme updated pour �viter de recommencer cette recherche par la suite
         ...
         {
            char* foundName = NULL;
            char* foundComputedName = NULL; // sert � prioriser les mises � jour de non computed
            char* prmName;
            for (prmName = DM_ENG_ParameterData_getFirstName(); prmName!=NULL; prmName = DM_ENG_ParameterData_getNextName())
            {
               if (((len==0) || (((int)strlen(prmName)>len) && (strncmp(prmName, name, len)==0))) // filtre les sous-param�tres de param
...
///// solution optimis�e pour r�duire le nb de parcours complets du data model  /////
                     if (((current->loadingMode & 1) == 0) && (current->storageMode != DM_ENG_StorageMode_COMPUTED))
                     {                           // c'est n�cessairement une feuille : la valeur peut �tre obtenue isol�ment,
                                                 // on le fait en cours d'it�ration pour une meilleure efficacit�
...
/////////////////////////////
            ...
            if (count++ > _MAX_SYSTEM_ENTRY) // pour d�tecter les boucles infinies
            {
               res = DM_ENG_INTERNAL_ERROR;
   ...
   {
      DBG("_loadSubParameters - withvalues is true");
      res = DM_ENG_ParameterManager_loadLeafParameterValue(param, false);
   }
   return res;
}
...
/*
 * cherche le proto le plus profond (on cherche un n� d'instance de droite � gauche) dont name serait une instance
 * retourne -1 si pas de proto, sinon l'index de la sous-cha�ne ".."
 */
   ...
   while (i>=0)
   {
      ...
      else
      {
         if (name[i]=='.')
         {
            ...
            else // on a trouv� un noeud chiffr� : condition 0 <= i < k-1 && name[i]=='.' && name[k]=='.' && chiffres entre les 2
            {
               int j;
               for (j=i; name[j]!='\0'; j++)
               {
...
{
   DBG("DM_ENG_ParameterManager_loadLeafParameterValue, param name is %s", param->name);
   int res = 0;
...
{
  DBG("DM_ENG_ParameterManager_loadSystemParameter");
   int res = 0;
   if (DM_ENG_Parameter_isLoaded(param, withValues)) return 0; // donn�es d�j� � jour, pas la peine de continuer

   // cas des donn�es group�es : on cherche l'objet racine du groupe
   if ((param->loadingMode & 1) != 0)
   {
      DBG("loadingmode is 1 -> _loadFinal");
      ...
      do
      {
         ...
         if (len <= 0) break; // sortie normalement irr�alisable
         parentName[len+1] = '\0';
   ...
   {
      DBG("loadingmode is not 1, loadsubparameters");
...
{
   DBG("DM_ENG_ParameterManager_loadSystemParameters");
   ...
   if (param == NULL)
   {
      // Le param�tre n'est pas stock� dans la base mais ce peut �tre une instance syst�me.
      // On cherche donc un prototype dont ce pourrait �tre une instance.
      char* proto = strdup(name);
      do
      {
         int i = _computeProto(proto); // i index de la 1�re sous-cha�ne ".." du proto obtenu
         if (i<=0) break;
      ...
      if (param == NULL)
      {
         ...
         return DM_ENG_INVALID_PARAMETER_NAME; // nom invalide : aucun param�tre, objet ou proto avec ce nom
      }
   }
...
{
   DBG("Parameter Manager: ParameterName is %s", name);
   DM_ENG_Parameter* param = DM_ENG_ParameterData_getParameter(name);
...
static int _setParameterValue(DM_ENG_Parameter* param, char* value, char** pData)
{
   ...
   if (res == 0) { res = _loadSystemParameter(param, pData);} // charger la valeur pr�c�dente pour �tre capable de la restituer en cas d'erreur
...
int DM_ENG_ParameterManager_setParameterValue(DM_ENG_EntityType entity, const char* name, DM_ENG_ParameterType type, char* value, OUT DM_ENG_ParameterStatus* pResult, OUT char** pSysName, OUT char** pData)
{
   ...
   if ((param == NULL) || DM_ENG_Parameter_isNode(param->name)
     || ((*pResult == DM_ENG_ParameterStatus_UNDEFINED) // v�rification uniquement pour le 1er appel (non r�cursif)
          && DM_ENG_Parameter_isHidden(param) && (entity != DM_ENG_EntityType_SYSTEM))) // param�tre cach�, c'est comme s'il n'existait pas
   {
   ...
   else
   {
      ...
      if (DM_ENG_Parameter_isBeingEvaluated(param)) // Pour �viter les boucles infinies
      {
         res = DM_ENG_INTERNAL_ERROR;
      }
      else
      {
         ...
         char* nextName = NULL; // nom pour set r�cursif dans le cas des redirections
         if ((param->storageMode == DM_ENG_StorageMode_COMPUTED) && (param->definition != NULL))
         {
   ...
   *pStatus = (param->immediateChanges ? DM_ENG_ParameterStatus_APPLIED : DM_ENG_ParameterStatus_READY_TO_APPLY);
   ...
   if (param->storageMode == DM_ENG_StorageMode_DM_ONLY)
   {
      if ((unsigned int)param->minValue >= INT_MAX) return DM_ENG_RESOURCES_EXCEEDED;
        // le nouveau numero d'instance est tjrs incr�ment�. Une fois le numero INT_MAX atteint, il n'est plus possible de cr�er
        // de nouvelles instances => mettre en place un m�canisme pour la r�utilisation des instance numbers lib�r�s
      param->minValue++;
...
int DM_ENG_ParameterManager_deleteObject(DM_ENG_EntityType entity, char* objectName, OUT DM_ENG_ParameterStatus* pStatus)
{
   ...
   *pStatus = (proto->immediateChanges ? DM_ENG_ParameterStatus_APPLIED : DM_ENG_ParameterStatus_READY_TO_APPLY);
   ...
   {
      // Suppression effective du param�tre
      res = DM_ENG_Device_deleteObject(proto->name, instanceNumber);
   }

   if ((res == 0)
    || (res == DM_ENG_INVALID_PARAMETER_NAME)) // L'instance n'est pas pr�sente au niveau syst�me, elle ne doit pas �tre dans le data model non plus
   {
...
void DM_ENG_ParameterManager_commitParameter(const char* name)
{
   ...
   char* nextName = NULL; // nom pour commit r�cursif dans le cas des redirections
   if ((param->storageMode == DM_ENG_StorageMode_COMPUTED) && (param->definition != NULL))
   {
      nextName = DM_ENG_Parameter_getLongName(param->definition, name);
   }
   ...
   if (DM_ENG_Parameter_isDiagnosticsParameter(name, &objName))
   {
      ...
      {
         pvs->type =(DM_ENG_ParameterType) 1; // type mis � 1 si le State est positionn� � Requested (sinon il reste � 0)
      }
      free(objName);
   }
   ...
   else // param�tres de contr�le des objets 'statistiques'
   {
      int k = strlen(name)-1;
      while ((k>0) && (name[k]!='.')) { k--; }
      if (k>0)
...
void DM_ENG_ParameterManager_unsetParameter(const char* name)
{
   ...
   bool cancelled = DM_ENG_Parameter_cancelChange(param); // cancelled permet d'�viter les boucles infinies
...
/////////// Acc�s � des param�tres particuliers ////////////
...
{
   char* res;
   DBG("DM_ENG_ParameterManager_getManufacturerName()");
...
////// Acc�s aux donn�es persistantes (en plus des param�tres) ///////
...
/**
 * Retourne l'�ch�ance la plus proche, �v�nement PERIODIC ou SCHEDULED, ou t�l�chargement planifi�
 */
time_t DM_ENG_ParameterManager_getNextTimeout()
{
   ...
   DM_ENG_TransferRequest* tr = DM_ENG_ParameterData_getTransferRequests();
   while (tr != NULL) // on recherche le 1er transfert non d�marr�
   {
...
/*
 * Traite une requ�te de t�l�chargement
 */
        ...
        unsigned int fileSize, char* targetFileName, char* successURL, char* failureURL, char* commandKey, OUT DM_ENG_TransferResultStruct** pResult)
{
   ...
   if (delay == 0)
   {
      ...
      if (res!=1) // requ�te trait�e de fa�on synchrone (avec succ�s ou non), on la supprime de la file
      {
         time(&comp);
         DM_ENG_ParameterData_removeTransferRequest(tr->transferId);
      }
...
/*
 * Construit la table des requ�tes de t�l�chargement dans la file
 */
...
// file des notifications de changements par le syst�me
...
/**
 * Verrouille la ressource "param�tres" si disponible
 * @return true si la ressource a pu �tre verrouill�e
 */
static bool tryLock()
{
...
/**
 * Demande de d�verrouillage de la ressource "Param�tres"
 * Si les donn�es ont �t� modifi�es, celles-ci sont tout d'abord sauvegard�es.
 * Si la file d'attente des notifications syst�me n'est pas vide, celles-ci sont trait�es en priorit�.
 */
void DM_ENG_ParameterManager_unlock()
{
   ...
   // effacement des valeurs des param�tres SYSTEM_ONLY
   DM_ENG_Parameter* param;
   for (param = DM_ENG_ParameterData_getFirst(); param!=NULL; param = DM_ENG_ParameterData_getNext())
   {
   ...
   // on traite prioritairement les changements notifi�s par le syst�me avant de d�verrouiller
   while (true)
   {
   ...
   // contr�le des changements sur les param�tres computed
   char* prmName;
   for (prmName = DM_ENG_ParameterData_getFirstName(); prmName!=NULL; prmName = DM_ENG_ParameterData_getNextName())
   {
...
/*
 * Lance un reboot si demand� (cette fonction doit �tre appel�e hors session ou en fin de session)
 */
...
/**
 * Cette m�thode doit �tre appel�e lorsque les data sont verrouill�es.
 * Elle permet la sauvegarde des donn�es avant d�verrouillage au cas o� on aurait un plantage
 * ou un red�marrage avant un unlock().
 *
 * Dans le m�me temps, les valeurs calcul�es sont effac�es. Cette fct �tant appel�e apr�s les modifs apport�es au data model (via les RPC
 * SetParameterValues, AddObject etc.), cela permet de r�actualiser les valeurs lors d'un prochain acc�s par GetParameterValues.
 */
void DM_ENG_ParameterManager_sync(bool resetComputed)
{
   if (resetComputed)
   {
      // effacement des valeurs calcul�es
      DM_ENG_Parameter* param;
      for (param = DM_ENG_ParameterData_getFirst(); param!=NULL; param = DM_ENG_ParameterData_getNext())
      {
...
//////////////////////////////////////////////////////////////
/////////// Impl�mentation ParameterBackInterface ////////////
//////////////////////////////////////////////////////////////
...
static bool _checkValueChange(DM_ENG_Parameter* param)
{
   ...
   {
      if (param->value == NULL) // inconnue => on consid�re qu'on a un changement
      {
         res = true;
         DM_ENG_Parameter_updateValue(param, NULL, true); // efface la valeur et positionne VALUE_CHANGED
      }
      else if (param->storageMode != DM_ENG_StorageMode_DM_ONLY)
      {
         ...
         if (codeRetour==0)
         {
            if (actualValue==NULL) { actualValue = (char*)DM_ENG_EMPTY; } // NULL interpr�t� comme Empty
...
static void processChange(const char* name)
{
   ...
   if (param != NULL)
   {
      ...
      {
         // utile si l'URL est stock� au niveau syst�me
...
void DM_ENG_ParameterManager_dataValueChanged(const char* name)
{
   ...
   else // on place les noms de param�tres dans une file d'attente
   {
      DM_CMN_Thread_lockMutex(_changesMutex);
...
void DM_ENG_ParameterManager_dataNewValue(const char* name, char* value)
{
   ...
   if (acquired)
   {
      ...
      DBG("name is %s, new value is %s", name, value);
...
void DM_ENG_ParameterManager_updateParameterNames(const char* name, DM_ENG_ParameterValueStruct* paramList[])
{
   if (paramList != NULL)
   {
      ...
      while (true); // dans le cas o� il y a eu un objet supprim�, on recommence
   }
}
...
void DM_ENG_ParameterManager_updateParameterValues(DM_ENG_ParameterValueStruct* paramList[])
{
   if (paramList != NULL)
   {
      ...
      for (j = 0; paramList[j] != NULL; j++)
      {
         ...
         {
            // Si cet �lement de la liste a un nom d'objet, on traite la demande, � partir de cet �l�ment
            // comme une mise � jour de l'objet (avec ajouts et suppressions possibles d'instances)
 ...
 * @param code Code r�sultat du t�l�chargement : 0 si OK, code d'erreur 90XX sinon
 * @param startTime Date de d�marrage du t�l�chargement
 * @param completeTime Date de fin du t�l�chargement
 */
 ...
 * @param fileSize Size of the file to be transfered in bytes
 * @param targetFileName File name to be used on the target file system
 * @param code 0 if the transfer was successful, otherwize � fault code
...
/**
 * Permet la synchronisation sur l'arr�t du scheduler
 */
