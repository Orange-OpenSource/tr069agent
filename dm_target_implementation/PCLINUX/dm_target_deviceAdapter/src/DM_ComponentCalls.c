...
// Séquencement des états
// Alternance entre épisodes sans perturbation (état 0) et épisodes avec perturbation (états 1 ou 2)
// _PROBA[1] et _PROBA[2] spécifient le % de chance de passer de l'état 0 à, respectivement, soit l'état 1 soit l'état 2  

// Settings

// durée médiane en ms
static const unsigned int _DURATION[][3]   = {{ 7500, 30, 40 },      { 1500, 1, 2 },       { 1000, 3, 30 },        { 1000, 3, 30 },        { 1500, 12, 35 },      { 1200, 5, 30 },        { 5000, 5000, 30 }};
// proba d'apparition sur 100 => cf. Séquencement des états
static const unsigned int _PROBA[][3]      = {{ 0, 50, 50 },         { 0, 90, 10 },        { 0, 60, 40 },          { 0, 60, 40 },          { 0, 90, 10 },         { 0, 10, 90 },          { 0, 99, 1 }};
// taux de réception dans le contexte
...
// fct aléatoire appliquée à la durée
...
char* DM_ComponentCalls_getComponentData(const char* name, const char* data)
{
     ...
     || (strcmp(name, "Services.STBService.1.ServiceMonitoring.MainStream.1.RTPEventStats.CurrentSample.QualityScoreBeforeEC")==0) )
   {
      ...
      // rate = weight / Duration   : nb d'événements par seconde
...
bool DM_ComponentCalls_setComponentData(const char* name UNUSED, const char* value)
{
   ...
   printf("*** Nouvelles conditions de réception : %d\n", _conditions+1);
   return true;
}
...
// génère un nombre compris entre 0 et 3*median, en visant median comme médiane
...
static bool _packetReceived()
{
   if ((_now.sec > _tChangeState.sec) || ((_now.sec == _tChangeState.sec)  && (_now.msec >= _tChangeState.msec)))
   {
      ...
      _currState = (_currState != 0 ? 0 : (p < _PROBA[_conditions][1] ? 1 : 2)); // détermine le changement d'état => cf. Séquencement des états
