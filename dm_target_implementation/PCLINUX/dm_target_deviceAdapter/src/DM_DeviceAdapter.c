  ...
  if(0 == strcmp(paramNameStr, "ManagementServer.ConnectionRequestUsername")) {
    ...
    DBG("ConnectionRequestURL is %s", *pVal);
      ...
      *pVal = strdup(nextSoftwareVersionStr);
      ...
      char* ipadr = DM_SystemCalls_getIPAddress();
      *pVal = (ipadr==NULL ? NULL : strdup(ipadr));

  }  else if (0 == strcmp(paramNameStr, "LAN.MACAddress")) {

      char* macadr = DM_SystemCalls_getMACAddress();
      *pVal = (macadr==NULL ? NULL : strdup(macadr));
