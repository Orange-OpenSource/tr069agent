...
#include <sys/stat.h>
#include <dirent.h>
#define FileList_PATH "./FileList/"
#define FileList_SHORT_NAME "FileList."
/* -------- PRIVATE DEFINTIONS ----------- */
...
static const char* DM_ENG_FILELIST_NAME = DM_PREFIX "FileList.";
...
void DM_ENG_Device_openSession() {
  ...
  _transferLock(); // exlusion mutuelle avec les t�l�chargements (pas obligatoire. A adapter � l'impl�mentation des download et upload)
...
int DM_ENG_Device_getValue(const char* name, const char* systemData, OUT char** pVal)
{
  ...
  *pVal = NULL;
  ...
  if(0 == strcmp(paramNameStr, "ManagementServer.ConnectionRequestUsername")) {
    ...
    if (tmpStr != NULL)
    {
      DBG("_retrieveValueFromInfoList success, value is %s", tmpStr);
      *pVal = strdup(tmpStr);
    }
    else
    {
 ...
 *
 * @param factoryReset Indicates if or not the device must perform � factory reset.
 */
...
{
   int res = 9003; // Diagnostic non support�
...
int DM_ENG_Device_getObject(const char* objName, const char* data, OUT DM_ENG_ParameterValueStruct** pParamVal[])
{
   ...
   else if ((strcmp(shortname, "LAN.Stats.")==0) && (data == NULL)) // on utilise cette version bouchonn�e seulement si data == NULL
   {
      time_t currentTime;
      time(&currentTime);
...
static int _getStringValue(char* linePtr, char** valueStr)
{
  ...
  *valueStr = val;
  ...
  if (val != NULL) // il y avait des car sp�ciaux dans la cha�ne
  {
     free(*valueStr);
     *valueStr = val;
  }
  ...
  do{
    ...
    if(false == _isCommentLine(currentInfoPtr->paramNameStr)) {
      if(0 == strcmp(nameStr, currentInfoPtr->paramNameStr)) {
          DBG("_retrieveValueFromInfoList value is %s", currentInfoPtr->paramValueStr);
...
static int _pingTest(DM_ENG_ParameterValueStruct* paramList[], OUT DM_ENG_ParameterValueStruct** pResult[])
{
   int res = 1; // par d�faut, r�sultat asynchrone
   ...
   int dscp = 0;
   int i;
   for (i=0; paramList[i] != NULL; i++) // on r�cup�re les valeurs qui peuvent �tre dans le d�sordre
   {
   ...
   {
      // DiagnosticsState positionn� � "Error_Internal"
      *pResult = DM_ENG_newTabParameterValueStruct(1);
      ...
      res = 0; // r�sultat fourni de fa�on synchrone
   }
   else
   {
      ...
      if (!foundHost)
      {
         // DiagnosticsState positionn� � "Error_CannotResolveHostName"
         *pResult = DM_ENG_newTabParameterValueStruct(1);
...
int DM_ENG_Device_getSampleData(const char* objName, time_t timeStampBefore, const char* data, OUT DM_ENG_SampleDataStruct** pSampleData)
{
   ...
   if (_splitSourceAndData(data, &src, &endData))
   {
      if (src == 0)
      {
         ...
         if (res == 0)
         {
            *pSampleData = DM_ENG_newSampleDataStruct(objName, parameterList, time(NULL), true, false); // N.B. continued � true. Pourrait �tre positionn� � !_starting, ou autre...
            DM_ENG_deleteTabParameterValueStruct(parameterList);
         }
         else
         {
            printf("Polling de donn�es statistiques sur objet inconnu : %s\n", objName);
         }
      }
      else
      {
...
static void _testAction()
{
   // notification spontan�e de nouvelles donn�es statistiques
